...
**Tech Stack:** React 19, TypeScript, Zustand/zundo, XYFlow, Vitest, Vite, SVG, IndexedDB persistence.
...
- [ ] Run `aube exec vitest run tests/scenario-document.test.ts` and confirm the new module import fails.
...
- [ ] Run the focused test and `aube run build`.
...
- [ ] Run `aube exec vitest run tests/snapshot-migration.test.ts tests/storage.test.ts` and confirm failures.
...
- [ ] Run focused tests and `aube run build`.
...
- [ ] Run `aube exec vitest run tests/flow-store-scenarios.test.ts` and confirm failures.
...
- [ ] Run focused tests, `aube test`, and `aube run build`.
...
- [ ] Run `aube test`, `aube run build`, and `git diff --check`.
...
- [ ] Run final `aube test`, `aube run build`, `git diff --check`, and `git status --short --branch`.
