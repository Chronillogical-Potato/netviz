...
describe("Video mode camera follow", () => {
  test("renders the active animation name through a compositor camera", async () => {
    ...
    expect(source).toContain("onlyRenderVisibleElements={!renderAll && !isVideoPlayback}");
    expect(source.match(/setViewport\(/g)).toHaveLength(1);
    expect(source).toContain("void setViewport(lastViewport)");
